...
describe('AuthService (unit)', () => {
  ...
  beforeAll(() => {
    ...
    process.env.JWT_SECRET =
      'test-secret-key-for-unit-tests-minimum-32-bytes-long';
  });
  ...
  beforeEach(() => {
    usersService = {
      ...
      attachWallet: jest.fn().mockResolvedValue({
        id: 'new-user-id',
        wallets: [{ address: '0x1' }],
      }),
    };
    ...
    web3 = {
      ...
      verifySignature: jest.fn(async (dto) => ({
        isValid: true,
        address: dto.address,
        message: dto.message,
      })),
  ...
  it('verifies SIWE happy path - creates user when not exists', async () => {
    (nonceService.get as jest.Mock).mockReturnValue({
      message: 'message-0xabc',
  ...
  it('verifies SIWE fails with missing nonce', async () => {
    ...
    await expect(auth.verifySiwe('0xabc', 'sig')).rejects.toThrow(
      'No active SIWE message found or message expired',
    );
  });
  ...
  it('verifies SIWE fails with invalid signature', async () => {
    ...
    await expect(auth.verifySiwe('0xabc', 'bad-sig')).rejects.toThrow(
      'Invalid signature',
    );
  });
