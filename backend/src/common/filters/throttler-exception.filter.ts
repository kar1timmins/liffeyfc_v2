...
export class ThrottlerExceptionFilter implements ExceptionFilter {
    ...
    if (path.includes('/login')) {
      ...
      message = 'Too many registration attempts. Please wait 5 minutes before trying again.';
      retryAfter = 300;
    } else if (path.includes('/refresh')) {
      message = 'Too many token refresh requests. Please wait 1 minute.';
      retryAfter = 60;
    } else if (path.includes('/auth')) {
      message = 'Too many authentication requests. Please slow down and try again in a moment.';
      retryAfter = 60;
    }
